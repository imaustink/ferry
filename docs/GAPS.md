...
- **Nodes can still read every pod and Secret, through `system:node`.** NetworkPolicy no longer needs anything from a node: the control plane's ferry-netpol compiles and serves each joined Mac's nodes their own pods' rules over mTLS with the kubelet's certificate, and the `ferry-node-netpol` ClusterRole is gone (deleted at `ferry up` and `token create`). But `control-plane/up.sh` binds the whole `system:node` ClusterRole to `system:nodes`, so `can-i list pods -A` and `list secrets -A` as a node are still `yes`. With that binding removed they were `no` and policy still worked; it stays because `ferry node add` runs its kubelets on the first node's certificate, which the Node authorizer alone refuses. See NETWORK-POLICY.md.
