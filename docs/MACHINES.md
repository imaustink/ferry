...
becomes a Ready node, and `kubectl delete` takes it away again. A pod that does
not fit causes a machine that fits it, and the machine gives the memory back
when it is done. Pods reach each other across both modes at the addresses
Kubernetes knows them by. Those are [milestones](#milestones) 1, 1b, 2, 3, 4, 5
and 6 below. GPU into machines (7) is not built. An installed ferry carries all
of it; `ferry machines enable` turns it on. See [INSTALL.md](INSTALL.md).

This document was written before any of it existed. It is kept as the design it
argued for, with each milestone marked as it landed, and corrected where
building it proved the design wrong. That happened twice, and both are recorded
in milestone 3.
...
Linux containers, declared as custom resources, sized explicitly, and
reconciled by a controller on the Mac. The two run in one cluster, and a pod
lands in one or the other by where it is scheduled.
...
spent on kernels, three quarters of the 32 GiB Mac it was measured on, before a
single workload runs. That made the real pod ceiling memory rather than the
hypervisor's 128-VM cap, and made `maxPods: 110` a promise the machine could
not keep.
...
the other side for real, containerd on overlayfs in one VM, and measured the two
against each other. The result is stronger than the case made here originally,
because two of experiment 13's limits turned out to belong to *its stand-in for
a shared kernel* rather than to shared kernels:
...
`vm-per-pod` pays another 384.8 MiB every time. So mode 2 saves one copy of
every image per container as well as one kernel per pod, and that saving grows
with replica count and with image size.

The 22-container ceiling went the same way. It came from giving each container
a block device, and with containerd 40 ran in one VM without complaint.

So the case for mode 2 is not that sharing a kernel is cheaper in the abstract.
It is that sharing a kernel lets ferry stop reimplementing what Linux already
does well, and layer sharing is the part worth the most.
...
metrics from the `pods` cgroup, and conntrack that is actually reconciled.
...
watches these. For each one it creates a VM, unpacks the image to a disk, hands
it a bootstrap configuration, and waits for the node to register. Delete the
resource and the VM goes away with it.

In practice a person writes few `Machine` resources. The provisioner described
below creates most of them, in response to pods that have nowhere to run. A
hand-written `Machine` is the bottom layer and the thing to build first, not the
interface anyone is expected to use day to day.

Nothing about this is novel. It is the Cluster API shape: a `Machine`, plus an
infrastructure provider that turns it into a VM. Keeping the spec close to that
vocabulary is deliberate. It is the model people already know from managed
Kubernetes, and it leaves room to become a real CAPI provider later without
redesigning the resource.
...
A machine that omits it takes the cluster's default, which is what every
machine had before it could choose: `os-crash` under a `power-loss` cluster and
...
boots. A FerryNodeClass `durability` that no longer matches a provisioned
machine is drift, and the machine is replaced the way it is for an image change.

This used to be `FERRY_NODE_DISK_SYNC`, one value for every machine on the Mac,
set on `ferry-node serve`. It is still the default, but no longer the only
answer.
...
API model anyway: machines are replaceable.
...
`SPARSEMEM_VMEMMAP`, so during boot Linux allocates a `struct page` for every
4 KiB of the configured ceiling, and it sizes several hash tables from total
RAM as well. Those pages are touched, and the finding above is that touched
pages never come back.
...
That is about **105 MiB fixed plus 2.5% of the ceiling**, paid at boot whether
or not a pod ever asks for it. On a real mode 2 cluster that is 356 MiB between
a 15 GiB node and the 2 GiB one ferry defaults to, 1,538 MiB of idle against
1,182, and it buys nothing measurable: the 20-pod burst is 1.22–1.26 s across
that whole range, medians of three runs. See
`experiments/24-benchmark-harness/m2-size-sweep.sh`.

So the balloon is worth having to enforce a limit, and a ceiling raised above
what the workload needs is not worth having.

This also shows how capacity has to be managed here. **Deleting a VM is the only
thing that returns memory to the host.** The balloon does not, and nothing else
can. So the answer to a node that is the wrong size is not to resize it. It is
to make another one and let this one go.

## Nodes on demand

A fixed set of hand-declared machines inherits the worst of both modes. Size
them small and workloads do not fit. Size them large and the Mac pays for memory
nobody is using, with no way to get it back. The resolution is to stop declaring
nodes and start deriving them: a controller that watches what the cluster cannot
schedule, creates a machine shaped to fit it, and removes machines whose memory
...
a node shaped for them. Since ferry cannot resize a VM, picking the right shape
at creation is the only option available.
...
3. It creates a `Machine`, the VM boots and joins, and the scheduler places the
   pods. Nothing simulates a node that does not exist, because by then it does.
...
treated as disposable, and a warm spare makes it feel instant.

### The host budget is the real constraint

Guest memory is lazily backed, so the sum of every machine's ceiling may exceed
the Mac's RAM and cost nothing until the guests touch it. After that there is no
way to un-touch it. Overcommitting ceilings is therefore safe to *advertise* and
dangerous to *use*, and the controller has to hold both numbers: what it has
promised, and what the host has actually backed. `spec.limits` is the promise
ceiling. The Mac's free memory is the real one, and a machine that would cross
it should fail to provision rather than take the Mac down with it.
...
about 125% of the Mac. Now both sides count the same promises the scheduler
counts, against the Mac's memory and from both directions: machines by
`spec.memory`, and the Mac's pods by their requests plus the `ferry-vm`
overhead.
...
- **The Mac node reserves what machines hold.** `ferry-machined` writes the sum
  ...
  machine arriving makes the Mac node smaller within about ten seconds, and one
...
What is left open is that nothing is enforced. A running pod on the Mac is
never evicted because a machine arrived. A Machine written by hand does not go
...
there, and nothing is taken from pods already running. Promised memory is not
the same as used memory either. This ledger adds up promises, and only the
kubelet's eviction thresholds see a Mac under real pressure.

Two smaller accounting notes. The 128-VM cap now bounds machines rather than
pods, which is no constraint at all at this scale, but in a mixed cluster pod
VMs and node VMs draw on the same 128. And each node VM has its own containerd
content store, so the same image pulled onto N machines is stored and cached N
times. A registry mirror on the Mac, which is already on the pod network, turns
that from N pulls into N copies of a local fetch.
...
**Stock containerd and kubelet, not `ferry-cri`.** Everything else in this
design depends on this decision. `ferry-cri` exists to make a pod out of a VM.
Inside a node VM, Linux already does that work, and reusing `ferry-cri` would
import every limit experiment 13 measured: the 22-container ceiling, the
duplicated image cache, and containers fixed at boot.
...
kube-proxy answers on each machine. That is the ordinary Kubernetes
arrangement, and it works here because a node VM has a kernel to program.

Mode 1's CoreDNS could not serve them, for the same vmnet isolation that shaped
the rest of this section. It is a `ferry-cri` pod on the Mac's vmnet network,
machines were on a vmnet network of their own, and a pod inside a machine had
no route to it.
...
anyway, because there are other reasons to want it. DNS on every machine is
node-local, answers without crossing to another node, and keeps a machine's pods
resolving when nothing else is up. What changes is that this is a choice rather
than a workaround.
...
Reusing those names would replace mode 1's DNS with a copy pinned to nodes it
does not have. Reusing that label would make the `kube-dns` Service
load-balance across pods half the cluster cannot reach, and DNS that works
intermittently is worse than DNS that does not work.
...
- The machine it provisions can never be reclaimed. Consolidation decides a node
  ...
  then only ever adds machines, which is the exact failure milestone 5 exists to
  prevent.

As a DaemonSet it does neither. DaemonSet pods do not drive provisioning, and
they do not hold a node against consolidation. Being per-machine is also the
better shape: DNS is local to the node asking, and no machine depends on another
to resolve a name.
...
`karpenter.sh/unregistered:NoExecute`, and removes the taint once it has synced
the NodePool's labels onto the node. The taint closes the window between a
kubelet registering and Karpenter finishing with it. A node is schedulable the
moment it registers, so anything the scheduler places in that gap is already
running somewhere Karpenter had not finished describing.
...
Karpenter logs that and proceeds, so machines worked and the race was invisible
until something lost it.

A taint patched on after the fact means nothing, which is the whole difference
between this and the mode label below. By the time a controller could apply it,
the node is already schedulable. So it comes from the kubelet, and travels the
whole way down. The provisioner writes `spec.node.taints` on the `Machine` it
creates, `ferry-machined` passes it to `ferry-node`, `ferry-node` puts it on the
kernel command line, and the guest's init hands it to `--register-with-taints`.
The string is the kubelet's own spelling from end to end, so nothing re-parses
it in between.

Two details keep it correct. Only machines the provisioner creates are tainted.
...
the kernel parameter that carries them. That is why the CRD refuses a taint
containing a space, rather than producing a machine that boots with a truncated
command line.

The NodePool's own taints take the same path, for the same reason: Karpenter
...
the taint *on* the node from outside is not practical, because Karpenter takes
it off within milliseconds of registration. So the absence of that error is the
assertion, and it is the same thing Karpenter itself checks.

## Which mode is the default, and what has to be true first

Mode 1 is the default today. The reason was provisioning rather than
confidence, and as of milestone 5 that reason no longer applies. The rest of
this section is the argument as it stood; what it asked for now exists.
...
from memory and a 16 GiB Mac advertises 61 pods (36 before experiment 32).
Mode 2's marginal container is ~17 MiB, and eight containers of one image cost
what one costs. Most people have less memory than the machine this was
developed on, and for them mode 2 is the difference between running their stack
and not.

What blocks it is not that mode 2 is newer. It is that **a `Machine` has to be
declared, with a size**, and ferry exists so that nothing has to be sized up
front. Defaulting to mode 2 as it stands would mean `ferry up` inventing a node
shape before knowing the workload, which is Docker Desktop's bargain with extra
steps. It would do that on the mode whose whole argument is density, where
guessing too small is a cluster that cannot schedule and guessing too large
wastes the memory you were trying to save.

Provisioning removes the guess rather than relocating it. A pending pod that
fits nothing creates the machine it needs, sized to fit, and consolidation gives
the memory back when it does not. At that point nothing is sized in advance in
either mode, and the claim at the top of the README holds for both. Mode 2
becoming the default is then a change of which node a pod lands on by default,
not a change in what ferry asks of you.
...
either mode. Mode 1 remains the default for the moment for two reasons. The
switch is a separate change that affects every existing cluster, whose pods
would start landing somewhere else. And milestone 6, the mixed cluster, is what
makes that landing predictable.

Mode 1 does not go away when mode 2 is the default. The Mac node is where the
provisioner runs.

## How a pod chooses

How to use this is in [RUNTIMES.md](RUNTIMES.md). What follows is how it came to
be built this way.
...
new admission behaviour to run, and the "negotiation" is ferry-cri comparing one
string. A RuntimeClass's `scheduling` is a `nodeSelector` and tolerations merged
into the pod at admission, so it selects the same label. `kubectl get nodes -L
ferry.dev/mode` still shows the truth, because the truth did not move.
...
- **Be the spelling people already know.** Kata and gVisor users ask for
  isolation with `runtimeClassName`, and now so can a ferry user.
- **Carry a toleration.** That is what makes a *default* possible: taint one
...
- **Refuse the wrong node.** Each class names a handler, `ferry-vm` for
  ferry-cri and `runc` for the machines' containerd, and the CRI says a runtime
...
`manifests/runtimeclasses.yaml` has both, and `ferry up` applies them.

**Built.** The Mac node's kubelet registers `ferry.dev/mode=vm-per-pod`, and
`ferry-machined` labels each machine `shared` once its node appears. `kubectl
get nodes -L ferry.dev/mode` shows both. The label was the missing half: this
document specified the selector before anything set it, so for a while the
selector above matched nothing on either side.

The controller patches the machine's label, rather than the machine's kubelet
registering it, which would be the race-free place for it. The kubelet inside a
machine is configured from the kernel command line. Putting a label there means
threading a value that is identical on every machine through `ferry-machined`,
`ferry-node`, the boot arguments and `init.sh`. The cost is a window of up to
one reconcile interval where the node is Ready and unlabelled, and a pod
selecting `shared` will not schedule there yet. That is the safe direction. The
label is missing rather than wrong, so the scheduler declines to place the pod
rather than placing it somewhere it does not belong.
...
the Mac's volumes folder. `ferry-node` shares that folder into every machine at
boot, and `init.sh` mounts it at the same path it has on the Mac. The volume is
pinned to `ferry.dev/host` rather than to one node, so a pod finds its data
again on a replacement machine or on the Mac. `ferry up` sets that label on the
Mac's node and `ferry-machined` sets it on each machine. Until this,
`ferry-storage` served only claims scheduled to the Mac's node, and a mode 2 pod
with a claim stayed Pending with no event. `chown` does not hold on these
volumes. virtiofs is served as the Mac user, and on macOS 26 it reports each
caller's own uid as the owner, so a chown appears to succeed and nothing is kept
...
ext4 image the machine takes over USB mass storage after it has booted. That is
the one device Virtualization.framework will attach to a running VM (experiment
26). `ferry-machined` decides which machine holds each disk, one at a time,
from the pods scheduled to them. `ferry-node` formats, labels and attaches it.
The node image's `ferry.dev/block` FlexVolume driver finds it by label, mounts
it once, and bind-mounts it into every pod on the machine that uses it. It
follows its pod to a replacement machine like a directory does. The cost is
synced small writes at about a third of the share's rate (0.43-0.64 ms a commit
against 0.13-0.16), which is why it is a class to ask for and not the default.

A ReadWriteOnce claim made for a pod on the Mac's own node is different. It is
an ext4 disk attached to that pod's VM, and pinned to the Mac's node by
...
`didn't match PersistentVolume's node affinity`. It stays where its data is, as
a local volume does anywhere. To move it, delete its claims and let them be
made again on the machine, which starts it with empty volumes. Or give it
ReadWriteMany claims, which are the shared directory on both.

### A default for pods that do not choose

**Built.** Kubernetes has no default RuntimeClass, so ferry makes the default
the way Kubernetes makes every "not here unless you ask": a taint.
`defaultRuntime` in ferry's config file picks which kind of node gets one.
...
Each RuntimeClass tolerates its own nodes' taint. A pod that names one gets it
whatever the default is, and a pod that names nothing can only go to the
untainted kind. ferry's own pods that belong somewhere particular say so: mode
1's CoreDNS tolerates the Macs' taint, machine CoreDNS tolerates the machines',
and the builder and the registry addon name `ferry-vm`.
...
the file. `ferry-machined` reads it every reconcile and does two things with it.
A machine created under `ferry-vm` registers with the taint, by the same
...
`ferry-vm` therefore changes the NodePool's template, which Karpenter treats as
drift. Each provisioned machine is replaced by one made under the new default,
and its pods move as consolidation would move them. Seen on a live cluster:
switching to `ferry-shared` marked the one machine drifted, and a replacement
...
cluster, so nothing moves until someone chooses one, and `ferry init` asks.
...
1. ~~**One machine, by hand.**~~ **Done.**
   ...
   were about virtualization: a read-only `/proc/sys`, eviction thresholds sized
   for a Mac rather than a 2 GiB root filesystem, no `iptables` for the CNI
   plugin, and no `/etc/hosts` for containerd to copy. It is *not* a node image.
   The software is staged into a `ferry-cri` pod VM, which is why those four
   caused trouble.
1b. **The node image.** **Done.**
   ...
   kernel command line. It is Ready in **13.8s**, and pods on it now reach the
...
2. ~~**`Machine` CRD and `ferry-machined`.**~~ **Done.**
   ...
   `Machine` and a node is Ready **16 seconds** later. `kubectl delete` it and it
   is gone in **3**: VM stopped, `Node` removed, disk and token cleaned up behind
   a finalizer. `kubectl get machines` reports the address once the machine has
...
4. ~~**Provisioning on demand.**~~ **Done.**
   `ferry-karpenter` is Karpenter with ferry as its cloud provider. `Create`
   ...
   catalogue. The host budget is enforced by refusing with Karpenter's
   insufficient-capacity error. That is the one part of a cloud provider a cloud
   never has to write, because a region does not run out when you ask for one
   more node. `ferry machines enable` starts it beside `ferry-machined`.
   ...
   Six things had to be fixed before any of that happened. Five had the same
   symptom: a provisioner that starts cleanly, logs nothing wrong, and
   provisions nothing.
   - Karpenter will not build for a `NodePool` whose NodeClass is not Ready, and
     nothing sets that condition for you, so a provider needs its own status
     controller.
   - Its operator parses `os.Args` itself and exits on a flag it does not know,
     so ferry's settings come through the environment.
   - Its scheme is client-go's global one, so a type not registered there
     panics deep inside `NewControllers`.
   - `DeepCopy` on a type embedding `ObjectMeta` resolves to the embedded method
     and silently returns the wrong thing.
   - A `Node` with no `providerID` is a claim that never registers, so
     `ferry-machined` sets one.
...
5. ~~**Consolidation.**~~ **Done.** Cordon, drain honouring PDBs, delete, and
   the memory returns to the Mac. Without this half, provisioning only ever adds
   machines, and on a laptop that is a memory leak.
   ...
   gone. The cluster is back to the Mac node alone.

   Getting there found the one design flaw this pair had. Machine CoreDNS was a
   `Deployment` pinned to machines, which provisioned a machine on its own and
   then held it against consolidation forever. It is a `DaemonSet` now, for
...
6. ~~**Mixed cluster.**~~ **Done.** A pod on the Mac node and a pod inside a
   ...
   *cluster* prefix rather than its slice's, the arrangement mode 1's pods
   already use, and `proxy_arp` puts that machine's pods on the segment without
   giving each of them a port on it. A mode 1 pod treats the whole cluster CIDR
   as on-link and ARPs for what it wants. The machine answers for the addresses
   it routes to `cni0` and forwards what arrives. **Nothing on the mode 1 side
   needs a route**, which is what makes this work at all: those pods are VMs
   nobody can reconfigure once they are running.
   ...
   LAN address. That address *is* reachable from a machine, through vmnet's NAT,
   as the host, so the route looked reasonable and silently blackholed. It now
   only routes to nodes on its own segment, and leaves the rest to the switch.
   ...
   *from* a machine still arrived, because that direction only needs the address
   the datagram came from. Frames *to* a machine needed the peer set, so every
   ARP, which is how a conversation starts, was flooded to nobody. The symptom
   was a machine that could reach mode 1 while mode 1 could not reach it, which
   looks like a routing problem and is not one.
7. **GPU into machines.** A device plugin inside the node VM proxying to
   `ferry-gpud` over vsock. That is *more* standard than what mode 1 does
...
- **Whose control plane?** Three answers, and the middle one is usually skipped.
  ...
  *Clusters with native control planes*: ferry already runs two independent
  clusters on one Mac, with separate etcd, apiserver, controller-manager,
  scheduler, PKI, pod network and ports, keyed by checkout directory. A cluster
  therefore costs four processes rather than a VM, and only the workers need to
  be virtual. This is the managed-service experience (throwaway clusters, a
  cluster per branch, a 1.31 and a 1.34 control plane side by side) without
  giving up the native control plane.

  The destination is the third; the first step is the first. The third needs
  profiles promoted from a worktree-keyed convenience into a real interface,
  plus a resource to declare a cluster. Its binding constraint is vmnet: 32
  networks for the whole Mac, held for about a minute after use
...
- **Node image: build or adopt.** Building one keeps ferry's toolchain and
  ...
  how a cluster is formed. Building first is the lower-risk path, and the
  interface (`spec.image`) is the same either way.
- **CAPI compatibility.** Ferry-native CRDs now, or the real Cluster API
  ...
  and CAPI can wrap it later. The cost of getting the spec shape wrong now is a
  migration.
- **Write the provisioner, or drive Karpenter.** Karpenter's provisioning and
  ...
  *Running inside the cluster it provisions for* turned out not to happen at
  ...
  nothing that makes nodes needs a node to run on, and there is no Linux image
  to build and publish for a controller.

  *A catalogue rather than arbitrary shapes* is solved by synthesising one.
  `GetInstanceTypes` can enumerate shapes from the `machine.cpus` and
  `machine.memory` ranges above (powers of two within the range is enough), and
  Karpenter will bin-pack against them. A hypervisor does not care that the
  shapes came from a list.
  ...
  The argument against is real: Karpenter is a large dependency for a laptop.
  Its multi-zone, spot and instance-type arbitrage features mostly do not apply
  to one Mac with a handful of machines, while its operational cost applies in
  full: CRDs, a controller to keep running, and version skew with the
  Kubernetes it provisions for. A provisioner that said "pods are pending and
  do not fit, so make one machine big enough, respecting the budget; delete
  machines that have been empty for a minute" would be a few hundred lines.
  What argues against *that* is the second half. Drain honouring
  PodDisruptionBudgets is where the hard cases are, and Karpenter already
  handles them.
...
kernel and a node where a hundred pods share one, and a workload picks. Docker
Desktop, colima, kind and Orbstack cannot offer the first, and nothing that only
does the first can be dense.
