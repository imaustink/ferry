...
| `FERRY_BLOCK_CLASS` | `ferry-local-block` | the StorageClass whose ReadWriteOnce claims are disks on machines too -- attached over USB after boot, so `chown` works -- rather than directories in the virtiofs share. Offered only when the kernel carries usb-storage. Synced small writes are about a third as fast as the share's |
