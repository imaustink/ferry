...
extension HTTP2ServerTransport.Config {
  ...
  public struct HTTP2: Sendable, Hashable {
    ...
    /// The maximum rate at which a client may send PING, SETTINGS, PRIORITY, ALTSVC and ORIGIN
    /// frames before the connection is closed with `ENHANCE_YOUR_CALM`.
    ///
    /// Defaults to NIO's limit of 200 per 30 seconds. A busy grpc-go client can exceed that on a
    /// low-latency connection without misbehaving: its BDP estimator sends a PING each time it
    /// receives data while none is outstanding, which on a Unix domain socket is many times a
    /// second.
    public var controlFrameRateLimit: ControlFrameRateLimit = .defaults

    public struct ControlFrameRateLimit: Sendable, Hashable {
      /// The number of frames permitted within `window`.
      public var maximumCount: Int

      /// The sliding window over which frames are counted.
      public var window: Duration

      public init(maximumCount: Int, window: Duration) {
        self.maximumCount = maximumCount
        self.window = window
      }

      /// 200 frames per 30 seconds, NIO's default.
      public static var defaults: Self {
        Self(maximumCount: 200, window: .seconds(30))
      }
    }
