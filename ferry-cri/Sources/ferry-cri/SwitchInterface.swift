...
struct SwitchInterface: Interface, VZInterface, @unchecked Sendable {
    ...
    /// ferry's end of the pair. Frames the pod sends arrive here.
    let hostSide: FileHandle
    ...
    init(address: CIDRv4, mac: MACAddress?) throws {
        ...
        self.hostSide = FileHandle(fileDescriptor: fds[0], closeOnDealloc: false)
    ...
    /// Reports what the pod puts on the wire, which is all this spike needs to
    /// show: if frames arrive, ferry owns a datapath it can forward anywhere.
    func observe(label: String) {
        let handle = hostSide
        Thread.detachNewThread {
            var seen = 0
            while true {
                let frame = handle.availableData
                if frame.isEmpty { continue }
                seen += 1
                if seen <= 5 || seen % 50 == 0 {
                    let kind = frame.count >= 14
                        ? String(format: "0x%02x%02x", frame[12], frame[13]) : "short"
                    FileHandle.standardError.write(
                        "switch[\(label)]: frame \(seen), \(frame.count) bytes, ethertype \(kind)\n"
                            .data(using: .utf8)!)
                }
            }
        }
