...
OCI layout directory. Nothing needed inventing. ferry only had to call it.
...
The pod sets `imagePullPolicy: Never`, so nothing could have fetched it from
anywhere. Both an OCI layout directory and an archive of one work.
...
Desktop was stuck on this machine at the time, which is why the script exists.

## Known limits

- **One node.** `ferry image load` loads an image into the node it was run
  against. Another Mac in the cluster does not have it. `minikube` has the same property per profile.
