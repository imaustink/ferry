...
this "ferry has no CNI." Every other Linux-shaped dependency (the kubelet,
kube-proxy, CRI) got a clean seam. Why is this one different, and does it have
to be?
...
CNI is a *protocol*: JSON on stdin, JSON on stdout, the verb in the environment,
and one process per invocation. It is not a kernel API. Whether a given plugin needs
...
`libcni`, `pkg/invoke` and `pkg/types/100`, which make up the runtime rather
than the plugins, build for darwin/arm64 unmodified.
...
The lack of hotplug stops being a defect. It is the reason the IPAM/main split
...
- **hostPort** via upstream `portmap`. ferry does not implement it today:
  `host_port` appears only in the CRI proto.
...
The claim in the README changes from "we had to write our own" to a stronger
and more accurate one: ferry is a CNI runtime whose main plugin is a hypervisor.
...
**`ferry-cni`** is the CNI runtime, in Go, beside ferry's other components.
It holds the dispatching `invoke.Exec` described above:
...
**`ferry-vm`** is the main plugin, and deliberately almost empty. A main plugin
creates a netns and puts an addressed interface in it, and the hypervisor does
that. What is left is the part that belongs to CNI: describe the interface, and
...
default does not need an allocator, because something upstream of CNI already
allocated.

**hostPort's last hop is ferry's.** `portmap` makes the mapping real on the
pod's own addresses, but Kubernetes means the node's, which is this Mac.
ferry-cri writes the mappings down and ferry-proxy listens, forwarding to the
pod at the same port so the pod's own rule is what rewrites it. TCP goes
through the stream proxy, UDP through the datagram one.

## What is not proven

Everything above is measured. Everything here is reasoning:
...
- **Per-pod sysctls.** `tuning` is built and shipped, and it cannot work yet for
  ...
  the damage to that pod's own kernel. That is a stronger argument than Linux
  can make, and still an unexamined change.
- **Hairpin.** With `snat` off, a container reaching its own hostPort through a
  host address is untested.
- **`bandwidth`** is built and never invoked. Nothing in CRI asks for shaping.
...
2. **`firewall` as a second NetworkPolicy path**, now that the guest half is
   real. Compare it against what `ferry-netpol` compiles today.
...
- **A vmnet subnet can be shadowed by a route the Mac already has.** On a
  ...
  though the pods could reach it. ferry falls through its candidate list on
  vmnet errors, but it does not check whether a candidate is routable to the
  bridge it just made.
