...
that native is faster to start and faster to run. That is worth a number rather
than a belief.
...
- **Running**: the same etcd 3.6.5, the same `etcdctl check perf --load=s`, on
...
on APFS — the storage path a control plane in a node VM would have.
...
The VM side of this was not measured — there is no node image yet — but the
comparison is nearly arithmetic. The same four processes, the same code, plus a
...
one hit. The difference is latency, and the VM won it by 8x — which is the
...
host's own weak fsync against 0.620 ms for the guest's, the difference being the
...
Profiles exist so a second checkout can run its own cluster, so this lands on
exactly the workflow it was built for. Not fixed here — this experiment did not
set out to change the control plane.

## What this means for the decision

- **Speed is not the argument.** Starting: the metal saves about one VM boot out
...
- **Durability is the argument, and it favours the metal.** On the Mac, etcd
  ...
  control plane whose fsync does not reach the platter is a control plane that
  can lose the last writes on a host crash.
- **So keep the control plane on Darwin** — for simplicity, for having no node
  image in the critical path, and because it is honest about durability. Not
  because it is faster.
- **And note the same hazard for mode 2.** Anything stateful in a node VM —
  a workload's database, or etcd if a cluster ever gets a control-plane
...
`startup.sh` needs the control-plane binaries in `bin/` — symlinking them from
a built checkout is enough, `etcdctl` included. Without `etcdctl` the etcd
