...
machine. The split is ferry's existing one. Go talks to the API server and Swift
talks to `Virtualization.framework`, the same way `ferry-streamer` and
`ferry-cri` divide the work.
...
the token it has to create first. **3 seconds to delete**, which matters because
deleting a VM is the only thing that returns memory to the host
...
kubelet has registered. A machine that boots and never joins leaves that column
empty.
...
On delete it stops the VM and deletes the `Node` object, since otherwise the
scheduler keeps placing pods on a machine that no longer exists. It removes the
token and the disk, and only then drops the finalizer.
...
The API server refuses the edit through a CEL rule on the CRD, so the
controller never sees it. A controller that ignored the edit would leave
`kubectl get machine` describing a machine that does not exist. Refusing it
says what is true, which is that a running VM cannot be resized
...
apart, so those two nodes cannot reach each other, and neither can pods on them. There are also only 32 networks for the whole Mac
...
It is better to have that design answer now than after building routing that
assumes otherwise.

## What this means

- **Milestone 2 is met.** A node is now a declared object, and the shape the
  provisioner needs is in place. Something else can create `Machine` objects
  and this controller will boot them.
- **The later milestones build on this.** Milestone 4 is the provisioner
  deciding *which* machines should exist from pending pods, and milestone 5 is
  consolidation deleting them again. Both come down to writing and deleting
  these objects.
...
- **No `MachineSet`, no replicas, no rolling replacement.** Editing `spec` is
  now refused, which is the correct behaviour, but nothing replaces a machine
  for you.
