...
it. A second process asking for the same subnet is refused. So one
`ferry-node serve` hosts every machine rather than a process each, the way
`ferry-cri` hosts pod VMs, and the two nodes land on one subnet (`192.168.96.2`
and `.3`) instead of on separate networks.

### Routes, from the node list

Each node reads the Node list with the kubelet's own certificate and installs a
route to every other node's `podCIDR` via that node's address. That is
flannel's host-gw in a handful of lines, because the parts that make it hard
elsewhere (one segment, a CIDR per node) are already true here.
...
around it. It was a socket pair per machine and a userspace switch between them,
the arrangement mode 1 arrived at for pods.

**That conclusion was wrong, and the evidence for it was worthless.** The pod
being probed had exited immediately with `sh: httpd: not found`, because this
...
where vmnet rewrites the source address. That is the case `MachineSwitch` could
not serve. Mode 1's `PodSwitch` spans machines because it carries a UDP relay
and a peer list. This copy had neither, and forwarded only
...
- **Milestones 4 and 5**, provisioning from pending pods and consolidation,
  come down to writing and deleting `Machine` objects, which experiment 19
  showed works.
