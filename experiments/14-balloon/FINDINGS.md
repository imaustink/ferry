...
**Method.** `Virtualization.framework` has a memory balloon. The probe boots a
VM with one, has the guest dirty a large block and release it, then asks for the
...
It uses Virtualization.framework alone, with no Containerization and no
`ferry-cri`, and is built with `swiftc` rather than SwiftPM, so it does not
depend on the package manager half of the toolchain being in working order.
...
A 4096 MiB VM. The guest dirties 2048 MiB and releases it, then the host asks
for 512 MiB:
...
restore. The balloon works in both directions, in seconds. The guest binds the
driver (`virtio1 device=0x0005 driver=virtio_balloon`), and ferry's kernel
...
less than 1%, which is noise against the 3.5 GiB the guest had just given up.
...
So the balloon is a cap, not a reclaim. It changes what the guest believes it
may use. It does not hand memory back to macOS, at least not on a host with
...
setting that can change on a running machine.

That makes the useful direction the opposite of the obvious one. Untouched
guest memory is nearly free: experiment 03 measured 64 GiB configured costing
1.6 GiB resident, and a 4096 MiB pod VM that allocates 1024 MiB costs 1434 MiB,
not 4096. So a pod VM can be configured generously and ballooned down to what
the pod is actually allowed, leaving room to grow later without a restart.
...
all. Adding it is a change to how the configuration is built, not a new
capability.
...
The cgroup permits 2 GiB, but the machine has 512 MiB. The pod dies well inside
its own limit, and the failure is a guest OOM rather than anything Kubernetes
can explain. The code comment at `PodRuntime.swift:405` calls right-sizing "a
worthwhile refinement, not a correctness issue". On this evidence it is a
correctness issue.
...
- **Configure generously, balloon down.** Because memory is lazily backed, a
  ceiling well above the limit costs nothing until touched, and the balloon enforces the
