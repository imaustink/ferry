...
kind. That is the safe assumption, and a safe assumption costs throughput when
it is wrong. Which is it?
...
not move. Running them concurrently buys one percent and costs each of them
half its speed, which also makes every GFLOP/s number the daemon reports
...
- **compute** is Metal work, serialised, because the first measurement says
  concurrency there is worthless.
- **model** is the on-device model, independent, because the second says
  concurrency here is free.

Within a lane, everything works as before: round-robin across pods, priority,
time slices, deadlines, cancellation. Across lanes there is no scheduling. A pod
...
- **Only two kinds of work.** Metal compute and the system model. A third kind
  of work, such as a Core ML model, a video encode, or Metal with a different
  memory profile, would need its own measurement before assuming which lane it
  belongs in.
- **One size of matmul.** 4096x4096 saturates this GPU. A workload too small to
  fill it might share, in which case serialising the compute lane costs
  something. This was not measured.
...
It takes about a minute and needs no entitlement and no cluster. It talks to
Metal and the on-device model directly, so it measures the machine, not ferry.
