...
it — image, boot, and the runtime's own sandbox setup. **Six seconds is the
answer milestone 1 was asked for**, and it is the answer the provisioner design
wanted: a node can be created in response to a pending pod and the pod waits
about as long as a slow image pull. Warm pools are an optimisation, not a
requirement.
...
Scheduled by the cluster's own scheduler, pulled by containerd inside the VM
(4 MiB image in 1.17s), addressed by the CNI bridge, and `kubectl logs` reads
it back through the kubelet's API. With the image already present, three more
pods reached Running in **0.86s, 0.84s and 0.99s** end to end — which is
kubectl to scheduler to kubelet to containerd to CNI, not a CRI-level figure,
so it is not the same measurement as experiment 16's 45 ms per container and
...
- **No `iptables`.** The CNI bridge plugin needs it for `ipMasq`, and portmap
  needs it too; without it every sandbox fails to set up its network. Disabled
  both to get here, which costs outbound NAT from pods. A node image has to
  carry iptables or nftables with the legacy shim — ferry already ships `nft`
  into pods for the same reason.
- **No `/etc/hosts`.** containerd builds each sandbox's hosts file from the
  ...
  files; a container image need not.
