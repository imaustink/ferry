...
arrives later has had nowhere to go but a directory in a virtiofs share -- and
virtiofs is served as the Mac user, so `chown` on it fails with EPERM. That is
the whole reason mode 2 volumes cannot be chowned, and the reason mode 1 gives
a ReadWriteOnce claim to one pod VM at a time.

Virtualization.framework cannot hot-add a virtio device. It can, since macOS 15,
attach a USB mass-storage device to a live xHCI controller:
...
Unmounted, detached (`USB disconnect, device number 2` in the guest), attached
again: the file and its ownership were both there. The data lives in the image
...
removal came back as `sdb`, the old `sda` still held by its dead mount.

The **ext4 superblock's UUID** is stable, and it is ferry's to choose: ferry-cri
formats every image. Read on the Mac at byte 1024+0x68 and in the guest from the
device at the same offset, two attached disks matched exactly:
...
About 2,400 synced writes a second against virtio's 12,400. Plenty for a
development database; a fifth of the machine's own disk. Large sequential
writes were not measured meaningfully -- the image was 384 MiB and host caching
returned the rest -- and are not what a volume is usually bottlenecked on.
...
The guest did not panic or hang. Attached again (as `sdb`) and mounted:
...
- **ReadWriteOnce as Kubernetes defines it, in mode 2.** A claim's image is
  ...
  every pod there: one node, many pods, `chown` working. The image is on the
  Mac, so a replacement machine can take it once the old one lets go.
