...
API server, covering registration, authn/authz, scheduling, volumes and
controllers?
...
through `kubectl`. The runtime is still experiment 01's fake, so nothing
executes. This tests the kubelet and API server half.
...
`cadvisor_darwin.go` reports `OS-IMAGE: macOS 15.6.1`, `KERNEL-VERSION: 24.6.0`
and `darwin/arm64` from `sysctl`. Capacity is the real machine.

What this exercised end to end:

- **Registration.** The kubelet creates its own Node object, authenticating as
  `system:node:ferry-mac` in group `system:nodes` against the Node authorizer.
- **Heartbeats.** Lease objects in `kube-node-lease`, and node status updates.
- **Scheduling.** The scheduler binds pods to a macOS node like any other.
- **Volumes.** The kubelet assembles the projected ServiceAccount volume on
  the Mac's filesystem with the correct atomic-writer layout, holding a
  genuine signed JWT:
  ```
  .../volumes/kubernetes.io~projected/kube-api-access-XXXXX/..2026_09_12_.../token
  eyJhbGciOiJSUzI1NiIsImtpZCI6IlhRRFNpWnNU...
  ```
- **Controllers.** Deployment to ReplicaSet to 10 pods, all reaching Running.
...
kernel. `tmpfs` becomes an ordinary directory, `Unmount` removes it, and it
reports anything else as unsupported rather than silently ignoring it. This is
correct for the target architecture. A pod is a VM, so real mounts happen
inside the guest, and the host only assembles volume contents in a directory
to hand over.

One consequence to carry forward: on Linux, projected ServiceAccount tokens
live on tmpfs and never touch a disk. macOS has no tmpfs, so they land on an
ordinary (FileVault-encrypted) filesystem and are deleted on teardown.
...
filter fixed it. A real CRI implementation must get this right: the kubelet
uses those listings to decide container ownership, and an unfiltered answer
corrupts its view of the whole node.

## Patch size after two experiments
...
| static pod file watching unsupported | `file_linux.go` is pure fsnotify with no Linux API, so its build tag widened to `linux \|\| darwin` | errors gone, no new code |
...
Once eviction worked, it acted on the node straight away, and correctly:
...
The host disk was 97% full, below the default `nodefs.available<10%`
threshold, so the eviction manager tainted the node and evicted all 10 pods.
That is correct Kubernetes behaviour driven by real macOS statistics, a whole
kubelet subsystem working, not a bug. Keep it in mind when testing on a full
disk.
...
which does not exist on this platform. Node-level memory and disk signals
work. This one cannot without a host cgroup tree.

## Still outstanding

- The control plane advertises the LAN IP. It must become the vmnet gateway.
