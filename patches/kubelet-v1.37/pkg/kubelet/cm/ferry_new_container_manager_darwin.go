...
// between minors. v1.36 threads a context through NewContainerManager and v1.37
// leaves it alone, so this is v1.36's unchanged -- copied rather than shared
// because the per-minor overlay is chosen by exact minor, and a minor with no
// directory of its own is one nobody has checked.
//
// Nothing in the darwin implementation blocks, so the context is accepted and
// dropped, which is what upstream's own unsupported variant does.
package cm
