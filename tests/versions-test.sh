...
echo usb   > "$kernel_root/kernel/usb-storage.config"
...
kslim="$(kinputs)"
refuses "moves with the configuration"       test "$kslim" = "$kthird"
echo changed > "$kernel_root/kernel/usb-storage.config"
refuses "moves with the usb-storage fragment" test "$(kinputs)" = "$kslim"
